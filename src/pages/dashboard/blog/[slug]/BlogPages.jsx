...
const API_ROOT = "http://localhost:3000/blog";
      ...
      <ul className="space-y-4">
        {recentBlogs && recentBlogs.length > 0 ? (
          recentBlogs.map((blog, i) => (
            <li key={i}>
              <Link
                to={`/blog/${blog.slug}`} // Now using the correct slug
  ...
  useEffect(() => {
    const fetchPostAndData = async () => {
      ...
      try {
        ...
        // Fix 1: Add slug to each blog object from the list
        ...
        // Fix 2: Set recentBlogs with the new objects that contain the slug
  ...
  if (loading)
    return (
      <div className="w-full h-screen fixed z-50 flex flex-col justify-center items-center bg-white">
        <AiOutlineLoading3Quarters className="animate-spin text-green-500" size={38} />
      </div>
    );
  if (!post)
...
/>
            ...
            <div
              className="prose max-w-none prose-lg md:prose-xl text-gray-800 leading-relaxed"
