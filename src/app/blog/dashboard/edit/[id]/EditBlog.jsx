...
const API_BASE_URL = "https://diginote-3b4g.onrender.com";
...
// Renamed this component to avoid conflict with next/link
...
const ToolbarButton = ({
  ...
  const baseClasses =
    "flex items-center justify-center min-w-[36px] h-9 p-2 rounded-md transition-all duration-200 cursor-pointer gap-1 text-sm font-medium";
  const variants = {
    default: `bg-white text-gray-700 border border-gray-300 hover:bg-gray-50 hover:border-gray-400 ${
      active ? "bg-blue-600 shadow-md border-transparent text-white" : ""
    }`,
...
const Alert = ({ type, message, onClose }) => {
  ...
  return (
    <div
      className={`rounded-lg p-4 mt-6 flex items-center justify-between border ${typeClasses[type]}`}
    >
      <span>{message}</span>
      {onClose && (
        <button
          onClick={onClose}
          className="ml-4 bg-transparent border-none cursor-pointer opacity-80 hover:opacity-100"
        >
          <FaTimes />
        </button>
      )}
...
export default function EditBlog() {
  ...
  const id = pathname.split("/").pop(); // Get ID from URL path
  ...
  const handleUpdateBlog = async (e) => {
    ...
    try {
      ...
      setSuccess(
        `Blog ${id ? "updated" : "created"} successfully! Redirecting...`
      );
      setTimeout(() => router.push("/blog/dashboard"), 2000);
    } catch (err) {
      setError(
        "Failed to submit blog. Please check your connection and try again."
      );
    } finally {
      setIsUpdating(false);
    }
        ...
        <header className="bg-white border border-gray-200 rounded-lg shadow-sm p-6 mb-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button
                onClick={() => router.back()}
                className="flex items-center gap-2 text-gray-600 bg-transparent border-none cursor-pointer transition-colors hover:text-gray-800"
              >
                <FaArrowLeft />
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-1 bg-gray-100 text-gray-700 rounded-md border-none cursor-pointer transition-colors hover:bg-gray-200"
              >
                <FaEye />
        ...
        <form onSubmit={handleUpdateBlog} className="flex flex-col gap-6">
              ...
              {id && (
                <div className="relative">
                  ...
                  <label className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center cursor-pointer rounded-xl opacity-0 transition-opacity hover:opacity-100">
                    <span className="text-white font-medium bg-blue-600 px-4 py-2 rounded-lg shadow-md transition-colors hover:bg-blue-700">
                      Change Image
                    </span>
                    <input
                      type="file"
                      accept="image/*"
                      onChange={(e) =>
                        handleFileChange(e, setNewImage, setImagePreview)
                      }
                      className="hidden"
                    />
                  </label>
          ...
          <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6 flex justify-between items-center">
            <button
              ...
              className="flex items-center gap-2 px-6 py-2 rounded-lg border border-gray-300 text-gray-700 cursor-pointer transition-colors hover:bg-gray-50"
            >
              <FaTimes />
              <span>Cancel</span>
            </button>
            <div className="flex gap-3">
              <button
                type="button"
                onClick={saveContent}
                className="flex items-center gap-2 px-6 py-2 rounded-lg bg-gray-600 text-white cursor-pointer transition-colors hover:bg-gray-700"
              >
                <FaSave />
              ...
              <button
                type="submit"
                disabled={isUpdating || !blog.title.trim()}
                className="flex items-center gap-2 px-6 py-2 rounded-lg bg-blue-600 text-white min-w-[140px] justify-center cursor-pointer transition-colors hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isUpdating ? (
                  <>
